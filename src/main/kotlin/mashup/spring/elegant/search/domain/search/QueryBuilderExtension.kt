 ...
 *  영업 시간, 배달가능동 조건 추가
 */
fun BoolQueryBuilder.addAcceptableConditions(area: String, now: LocalDateTime): BoolQueryBuilder = this.must(matchQuery(ShopField.DELIVERY_AREA.field, area))
    .must(
        nestedQuery(
            ShopField.OPEN_HOUR.field, boolQuery()
                .must(matchQuery(ShopField.OPEN_HOUR_WEEK.field, translateDay(LocalDate.now().dayOfWeek)))
                .must(
